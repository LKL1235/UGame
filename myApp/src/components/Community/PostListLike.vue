  ...
  <div class="main">
    <div class="posts" v-for="(post,index) in postNum">
      <div class="post" >
          ...
          <div>
            <a style="color: #FFFFFF;font-size: 11px;" @click="router.push({path:'/community/postListByBoard',query:{boardId:posts[index].boardId,boardName:posts[index].boardName}})">{{"来自："+posts[index].boardName}}</a>
          </div>
...
onMounted(()=>{
  axios.get("/getPostsLike" ,{params:{name:route.query.postName}}).then((respon)=>{
